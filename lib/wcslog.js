/**
 *  This is modified, unofficial fork of wcslog.js
 *  which supports Changing analytics parameters
 *  like `href`, `referer`, `search` for Single Page Applications.
 */
...
if (typeof wcs.norefresh == "undefined") {
    ...
    function ac(ax, aB, aA, aD) {
        var aC = [];
        aC[0] = "ci=" + aB;
        aC[1] = "t=" + Math.round(aA.getTime() / 1000);
        aC[2] = "u=" + q(w);
        aC[3] = "r=" + q(C);
        var aE = aC.join("|");
        aE = am.encode(aE);
        var az = new Date();
        az.setDate(aA.getDate() + 20);
        var ay = az.toGMTString();
        ae(ag, aE, ax, ay, aD);
        ae(ad, "1", ax, 0, aD);
        ae(P, aB, ax, ay)
    ...
    function y(ax) {
        ...
        if (ay) {
            aA = x(ay);
            if ((aA && aA.ci && aA.tr) && (aA.tr == "sa" || aA.tr == "cd" || aA.tr == "ft")) {
                ac(ax, aA.ci, az, aC)
            }
        }
    }
    ...
    wcs.setRefererHref = function (href) {
        C = href;
    };

    wcs.setHref = function (href) {
        w = href;
    };
